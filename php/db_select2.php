...
if ($mysqli) {
	...
	if (isset($decodedInput->id)) {
		$sql .= " where id like '" . $decodedInput->id . "%'";
	}
	if (isset($decodedInput->pg)) {
		$sql .= " limit 10 offset  " . ($decodedInput->pg - 1) * 10;
	}
	$result = mysqli_query($mysqli, $sql);
	$rows = mysqli_fetch_all($result, MYSQLI_NUM); //MYSQLI_ASSOC, MYSQLI_NUM, MYSQLI_BOTH, 
	//echo '{"sql":'. JSON_encode($sql) . ',"ds":' . JSON_encode($rows) . '}';
	echo '{"ds":' . JSON_encode($rows) . '}';
	mysqli_close(); 
} else {
